<template>
  <div id="playlist_detail" ref="playlistdetail">
    <div>
      ...
      <div class="songlist-bg-wrap gradient-wrap">
        <div class="songlist-info">
          <div class="songlist-img">
            <!--todo 加上v-if的话,初始化会默认playlist不为空,在获取之前，playlist.creator是未定义的-->
            <img class="gradient-img" v-if="JSON.stringify(playlist) !== '{}'" :src="playlist.creator.backgroundUrl" alt="" >
      ...
      <div class="song-list" >
        ...
        <div class="song-item" v-for="(item,index) in playlist.tracks">
          ...
          <div class="song-info" @click="playMusic(item.id)">
            <p class="song-name">{{item.name}}<span class="song-desc" v-show="item.alias[0]">{{item.alias[0]}}</span></p>
            <p class="song-singer"><span v-for="itemname in item.artists">{{itemname.name}}</span></p>
          </div>
...
//      歌单详情
      ...
      //获取歌曲信息，并统一存入vuex中
        axios.all([getSongUrl(), getSongDetail(), getSongLyric()])
